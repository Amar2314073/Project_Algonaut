...
function Profile() {
  ...
  const handleSave = async () => {
    try {
      ...
      // Make sure we're updating with the correct user data structure
      if (data && data.user) {
  ...
  const getCurrentImageUrl = () => {
    if (editMode) {
      return formData.image || "https://www.gravatar.com/avatar/?d=mp&s=200";
    }
          ...
          {editMode ? (
            <div className="w-full mt-4 flex flex-col gap-2">
              ...
              <div className="text-xs text-gray-500 mt-1">
                Enter a valid image URL (e.g., from imgur, gravatar, etc.)
              </div>
